...
class Tkinter_util():
    ...
    def generate_window_tk():
        ...
        label_title = Label(window, text="↑ ↑ ↑ Choisis ci-dessus si tu veux planter ou ramasser ↑ ↑ ↑", font=("Helvetica", 10), bg="green", fg ="white")
        ...
        frame_seed = Frame(self.window, bg='pink')
        frame_vegetable = Frame(self.window, bg='pink')

        # show the frame
        frame_seed.pack(expand=YES)
        frame_vegetable.pack(expand=YES)
...
if __name__ == "__main__":
    var.tkinter_inst = Tkinter_util()
    var.tkinter_inst.launch_loop()
